  ...
  function GeosMultiController($scope, $state, Authentication, geo) {
    ...
    function load_country(country) {
      ...
      d3.tsv(country_file, function (sm_data) {
        dvdf_v001.dc.utils.init_preprocessing('sm_data', TIME_DIM_NAME, DIM_FIELD_NAMES, VAL_FIELD_NAMES);
        sm_data.forEach(function (d) {
          dvdf_v001.dc.utils.process_data('sm_data', d);
          d.year = +d.year;
    ...
    for (_idx_csebassm_row=0; _idx_csebassm_row<CSEBASSM_NB_ROWS; _idx_csebassm_row++) {
      ...
      for (_idx_csebassm_col=0; _idx_csebassm_col<CSEBASSM_NB_COLS; _idx_csebassm_col++) {
        chart_series_evolution_by_sex_age_small_multiples[_idx_csebassm_row][_idx_csebassm_col]
            = dc.seriesChart('#chart_series_evolution_by_age_' + _idx_csebassm_row + '_' + _idx_csebassm_col);
      }
    }
    ...
    d3.json('./custom-world-countries.json', function (countriesJSON) {
      ...
      d3.tsv('./OUTPUT_simply_mortalite_short_detailed_evolution.tsv', function (mortalite_data) {
        ...
        dvdf_v001.timer.utils.lap('Starting loading mortalite data for small multiples ...');
        d3.tsv('./OUTPUT_simply_mortalite_by_time_age_range_sex.tsv', function (sm_data) {
          dvdf_v001.timer.utils.lap('Finished loading mortalite data for small multiples ...');
          ...
          for (_idx_csebassm_row=0; _idx_csebassm_row<CSEBASSM_NB_ROWS; _idx_csebassm_row++) {
            ...
            for (_idx_csebassm_col=0; _idx_csebassm_col<CSEBASSM_NB_COLS; _idx_csebassm_col++) {
              sex_age_dim_gps[_idx_csebassm_row][_idx_csebassm_col] = sex_age_dim.group().reduce(
                  dvdf_v001.dc.utils.sm_1D_reduce_add('sex', ['k_deaths'],
                      _idx_csebassm_row, _idx_csebassm_col, 'age', IDX__AGE__INDICATOR_BOOLEAN_FUNCTION
                  ),
                  dvdf_v001.dc.utils.sm_1D_reduce_remove('age', ['k_deaths'],
                      _idx_csebassm_row, _idx_csebassm_col, 'age', IDX__AGE__INDICATOR_BOOLEAN_FUNCTION
                  ),
                  dvdf_v001.dc.utils.sm_1D_reduce_init('age', ['k_deaths'],
                      _idx_csebassm_row, _idx_csebassm_col, 'age', IDX__AGE__INDICATOR_BOOLEAN_FUNCTION
                  )
              );
            }
          ...
          chart_geo_world_map
              ...
              .colorDomain([0, 10000])
              .colorCalculator(function (d) { return d ? chart_geo_world_map.colors()(d) : '#ccc'; })
          ...
          for (_idx_csebassm_row=0; _idx_csebassm_row<CSEBASSM_NB_ROWS; _idx_csebassm_row++) {
            for (_idx_csebassm_col=0; _idx_csebassm_col<CSEBASSM_NB_COLS; _idx_csebassm_col++) {
              chart_series_evolution_by_sex_age_small_multiples[_idx_csebassm_row][_idx_csebassm_col]
                  ...
                  .margins({top: 10, left: 35, right: 5, bottom: 20})
              ...
              //        chart_series_evolution_by_sex_age_small_multiples[_idx_csebassm_row][_idx_csebassm_col].filter = function() {};
            }
          }
